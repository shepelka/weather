...
class HourlyAdapter(
...
) : RecyclerView.Adapter<HourlyAdapter.WeatherViewHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WeatherViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.hourly_adapter, parent, false)
    ...
    override fun onBindViewHolder(holder: WeatherViewHolder, position: Int) {
        ...
        if (position == 0) {
            holder.tvTime.text = context.getString(R.string.hourly_now)
        } else {
            holder.tvTime.text = TimeHelper.roundToNextWholeHour(item.date, item.timezone)
        }
